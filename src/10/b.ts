...
const pathOnly = Array.from(new Array(maxY + 1), () =>
...
while (!positionsAreSame(A.nextPos, B.nextPos)) {
  setCellAt(A.nextPos, cellAt(A.nextPos) || "X");
  setCellAt(B.nextPos, cellAt(B.nextPos) || "X");
  A = traverse({ startPos: A.nextPos, direction: A.nextDirection });
  B = traverse({ startPos: B.nextPos, direction: B.nextDirection });
}
setCellAt(A.nextPos, cellAt(A.nextPos) || "X");

const marked = pathOnly.map((row) =>
  row.map((char, id) => {
    if (char === ".") {
      const pathCrossesLeft = row
        .slice(0, id)
        .join("")
        .replaceAll("-", "")
        .replaceAll("FJ", "|")
...
console.log(pathOnly.map((x) => x.join("")).join("\n"), "\n");

const markedString = marked.map((x) => x.join("")).join("\n");

console.log(marked.map((x) => x.join("")).join("\n"));

console.log(markedString.replaceAll(/[^I]/g, "").length);
