...
const distributeJokers = (spread: number[]) => {
  const nJokers = spread[0];
  spread[0] = 0;
  const highestCount = spread.reduce((result, next, id) => {
    if (next > spread[result]) {
      result = id;
    }
    return result;
  }, 0);
  spread[highestCount] += nJokers;
  return spread;
};

const spreadToType = (spread: number[]) => {
  if (spread[0] > 0) {
    spread = distributeJokers(spread);
  }
